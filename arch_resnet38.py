...
class Resnet38(nn.Module):
    def __init__(self, first_pool=False):
        super(Resnet38, self).__init__()

        self.conv1a = nn.Conv2d(3, 64, 3, padding=1, bias=False)
        self.firtst_pool=first_pool
    ...
    def forward(self, x):
        ...
        x, _ = self.b5(conv4, get_x_bn_relu=True)
        x = self.b5_1(x)
        conv5 = self.b5_2(x)

        x, _ = self.b6(conv5, get_x_bn_relu=True)

        x = self.b7(x)
        conv6 = F.relu(self.bn7(x))

        return (conv2, conv3, conv4, conv5, conv6)
